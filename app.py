...
def to_excel(df, results, inputs, company_name, valuation_date):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        wacc_df = pd.DataFrame({
            ...
            "Componente": ["Levered Beta", "Cost of Equity", "Cost of Debt (After Tax)", "Market Value of Debt", "Weight of Equity", "Weight of Debt", "WACC"],

            "Valor": [results['levered_beta'], results['cost_of_equity'], results['cost_of_debt'], results['market_value_of_debt'], results['weight_equity'], results['weight_debt'], results['WACC']]
...
# 2. Beta (Levered)
st.sidebar.subheader("2. Beta (Levered)")

beta_source = st.sidebar.radio(
    "Seleccionar origen de Beta",
    ["Beta de la compañía", "Beta sectorial (ponderada por ingresos)"],
    key="beta_source",
)

if beta_source == "Beta de la compañía":
    levered_beta = float_input(
        "Beta de la compañía (Levered Beta)",
        1.0,
        "levered_beta",
        format="%.3f",
    )
    ...
    if "beta_df" not in st.session_state:
        st.session_state["beta_df"] = pd.DataFrame([
            {"Sector": "", "Ventas": 0.0, "Beta Levered": 0.00}
        ] * 5)

    edited_beta_df = st.sidebar.data_editor(
        st.session_state["beta_df"],
        num_rows="dynamic",
        key="beta_editor",
    )
    ...
    levered_beta = (
        (edited_beta_df["Ventas"] * edited_beta_df["Beta Levered"]).sum()
        / total_ventas
        if total_ventas > 0
        else 0.0
    )
    st.sidebar.info(f"Levered Beta Ponderada: **{levered_beta:.4f}**")
...
use_country_risk = st.sidebar.checkbox("Considerar riesgo país (λ, CRP)", value=False)
...
# Build export_data from session state
...
for k, v in st.session_state.items():
    ...
    if k not in ['df', 'results', 'mc_stats', 'beta_editor', 'erp_editor']:
        ...
        export_data[k] = v.to_dict('records') if isinstance(v, pd.DataFrame) else v

# Export the ERP dataframe when multiple‑region ERP is selected

if erp_calc_method == "Múltiples Países/Regiones (Ponderado)" and "edited_erp_df" in locals():

    export_data["erp_df"] = edited_erp_df.to_dict('records')

# Export the sector beta dataframe when sector source is selected

if beta_source == "Beta sectorial (ponderada por ingresos)" and "edited_beta_df" in locals():
...
if 'df' in st.session_state and 'results' in st.session_state:
    ...
    with tab1:
        ...
        c4.metric("levered Beta", f"{results['levered_beta']:.4f}")
    ...
    st.info("Adjust the values on the left sidebar and click 'Run Valuation' to see the results.")